...
pub struct Socket {
    ...
    /// taken from it. Written to every outgoing segment's acknowledgement field so
    /// a middlebox sees a real receiver's ACK climb. Safe to advance because our
    /// decoy kernel is frozen with TCP_REPAIR and so emits no competing ACK of its
    /// own on this 4-tuple.
    ack: AtomicU32,
    /// Payload bytes received since our last bare ACK. Drives `send_ack` when there
    /// is no outgoing data segment to carry the advancing ACK.
...
impl Socket {
    ...
    fn new(
        ...
        // A listener passes the frozen kernel's numbers, so it is calibrated at
    ...
    fn build_tcp_packet_inner(
        ...
        // large transfers stay in-window. Both kernels are frozen with TCP_REPAIR,
        // so neither emits a competing ACK that our climbing value could contradict.
        ...
        loop {
            match self.state.load() {
                State::Established => {
                    ...
                    if !payload.is_empty() {
                        // Advance our receive cursor; this is what outgoing segments
                        // acknowledge, so a middlebox sees a real receiver's ACK
                        // climb. Move forward only, and only within half the sequence
                        // space, so a stale or reordered segment can't drag it back.
                        let new_ack =
                ...
                State::SynSent => {
                    ...
                    if (tcp_packet.get_flags() & expected_flag) == expected_flag {
                        // found our SYN + ACK. These seed the connector well enough to
                        // start; `adopt_kernel_state` then replaces them with the
                        // frozen kernel's authoritative numbers.
                        self.seq
                            .store(tcp_packet.get_acknowledgement(), Ordering::Relaxed);
                        // The SYN-ACK's sequence number is the peer kernel's ISN, so
                        // ISN+1 is the baseline our receive cursor advances from.
...
impl Stack {
    ...
    ///
    /// A listener passes `Some(kernel_state)` with the frozen kernel socket's
    /// authoritative sequence numbers. A connector passes `None`: it allocates in
    /// `SynSent` before its handshake completes, calibrates from the captured
    /// SYN-ACK, then adopts the kernel's numbers via `adopt_kernel_state`.
...
mod tests {
    ...
    fn mock_socket() -> (Arc<MockTun>, Stack, Socket, flume::Sender<Bytes>) {
        mock_socket_seeded(5000, 1000)
    }

    fn mock_socket_seeded(
        seq: u32,
        ack: u32,
        ...
        assert!(socket.seq_calibrated.load(Ordering::Relaxed));
        let tx = stack
            .shared
            .state
            .read()
    ...
    /// With our kernel silenced by TCP_REPAIR, the ACK field advances with received
    ...
    async fn repair_path_advances_the_acknowledgement_field() {
        ...
        let (_tun, _stack, socket, tx) = mock_socket_seeded(5000, 1000);
    ...
    /// A connector calibrates its SEQ and receive cursor from the captured SYN-ACK
    /// (before `adopt_kernel_state` later overwrites them with the frozen kernel's
    /// authoritative numbers), so it is ready to send once the handshake is seen.
    #[tokio::test]
    async fn connector_calibrates_from_syn_ack() {
        let tun = Arc::new(MockTun::default());
        let stack = Stack::new(tun.clone(), None);
        let local: SocketAddr = TEST_LOCAL.parse().unwrap();
        let remote: SocketAddr = TEST_REMOTE.parse().unwrap();
        ...
        // RCV.NXT = peer ISN (7000) + 1.
        assert_eq!(socket.ack.load(Ordering::Relaxed), 7001);
    ...
    /// Received payload drives when a bare ACK is emitted (there may be no outgoing
    /// data segment to carry the advancing ACK).
    #[tokio::test]
    async fn bare_ack_is_driven_by_received_payload() {
        ...
        // RCV.NXT (1000) advanced by the 4 payload bytes.
        assert_eq!(tcp_pkt.get_acknowledgement(), 1004);
